...
for i in $(seq 1 "$COUNT"); do
  ...
  for t in $(seq 1 150); do
    SA_OK="$(docker exec "$CLIENT_CONT" sh -lc "swanctl --list-sas | grep -q 'child: net' && echo ok || true")"
    if [ "$SA_OK" = "ok" ]; then
      if docker exec "$CLIENT_CONT" sh -lc "ping -c1 -W1 $SERVER_TUN_IP >/dev/null 2>&1"; then
        END_NS="$(date +%s%N)"
  ...
  PING5="$(docker exec "$CLIENT_CONT" sh -lc "ping -c5 -i 0.2 -W2 $SERVER_TUN_IP 2>/dev/null" || true)"
