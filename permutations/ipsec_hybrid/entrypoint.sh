...
echo "[ipsec_hybrid] Starting role=$ROLE" | tee -a "$LOG"

echo "[ipsec_hybrid] OpenSSL version: $(/usr/local/openssl-3.5/bin/openssl version)" | tee -a "$LOG"

sync_role_configs() {
  local role="$1"
  
  echo "[ipsec_hybrid] Syncing $role configuration..." | tee -a "$LOG"
  ...
  echo "[ipsec_hybrid] PKI files synced" | tee -a "$LOG"
}
...
echo "[ipsec_hybrid] Loading swanctl configuration..." | tee -a "$LOG"
for t in $(seq 1 5); do
...
echo "[ipsec_hybrid] Verifying ML-KEM availability..." | tee -a "$LOG"
if swanctl --list-algs 2>/dev/null | grep -q "ML_KEM"; then
    ...
    echo "[ipsec_hybrid] Available DH groups (sample):" | tee -a "$LOG"
...
if [ "$ROLE" = "client" ]; then
  echo "[ipsec_hybrid] Client will initiate connection on demand" | tee -a "$LOG"
fi
